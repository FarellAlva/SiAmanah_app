...
import 'package:zakatapp/src/features/result_screen_2.dart';

class RegKtp extends StatefulWidget {
  final String selectedTempat;

  const RegKtp({super.key, required this.selectedTempat});

  @override
  State<RegKtp> createState() => _ScannerKtpState();
}

class _ScannerKtpState extends State<RegKtp> with WidgetsBindingObserver {
    ...
    try {
      ...
      if (extractedNik == 'NIK not found') {
        ...
        await navigator.push(
          MaterialPageRoute(
            builder: (BuildContext context) => ResultScreen2(
              text: extractedNik,
              // Kirimkan UUID pengguna
            ),
          ),
        );
      }
